...
static void __attribute__((picinterrupt(("")))) myIsr(void)
{
    uint8 u8_counter;
    for(u8_counter=0;u8_counter<2;u8_counter++)
    {
        if ( 1 == ((((* (ast_INTS[u8_counter].REGISTER)))&(ast_INTS[u8_counter].MASK))/(ast_INTS[u8_counter].MASK)))
        {
           ast_INTS[u8_counter].aptrf_INT();
           ((* (ast_INTS[u8_counter].REGISTER)))&=~(ast_INTS[u8_counter].MASK);
           break;
        }
    }
}
