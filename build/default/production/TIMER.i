...
static uint8 au8_Timer_Started[]={0,0,0};
...
void Timer_Init(str_Timer_cfg_t* st_Timer_cfg)
{
 switch(st_Timer_cfg->Timer_CH_NO)
 {
  case (0) :
   switch(st_Timer_cfg->Timer_Mode)
   {
    case(0) :
     ...
     if(st_Timer_cfg->Timer_Prescaler == 1)
                    {
                    ...
                    else
                    {
                        ...
                        (OPTION_REG)|=(au8_Timer0_PrescalerTable[st_Timer_cfg->Timer_Prescaler]) ;
                    }
     break;
   ...
   au8_PrescalerValue[0]=au8_PrescalerValueTable[st_Timer_cfg->Timer_Prescaler];
   break;
  ...
  case (1) :
    ...
    case (2):
     ...
     break;
            ...
            switch(st_Timer_cfg->Timer_Polling_Or_Interrupt)
            {
                case 1:
                    (PIE1)|=(0x01);
                    if(((void *)0) != st_Timer_cfg->aptrf_INT)
                    {

                        INT_SetCallBackFunc(1,&PIR1,0x01,st_Timer_cfg->aptrf_INT );
                    }
                    break;
   ...
   au8_PrescalerValue[1]=au8_PrescalerValueTable[st_Timer_cfg->Timer_Prescaler];


            (T1CON)|=(au8_Timer1_PrescalerTable[st_Timer_cfg->Timer_Prescaler]);
   break;
  ...
  case (2) :
   ...
   au8_PrescalerValue[2]=au8_PrescalerValueTable[st_Timer_cfg->Timer_Prescaler];
   break;
 ...
 switch(u8_Timer_CH_NO)
 {
  case 0 :
   ...
   else
   {
    ...
    au8_Timer_Started[0]=1;


                u16_Timer_Count = 255 - u16_Timer_Count;
  ...
  case 1 :
   ...
   else
   {
                ...
                u16_Timer_Count = 65535 - u16_Timer_Count;


                TMR1L = u16_Timer_Count & 0x00FF;
                TMR1H = (u16_Timer_Count & 0xFF00)>>8;


    au8_Timer_Started[1]=1;
 ...
 switch(u8_Timer_CH_NO)
 {
  case 0 :
   ...
   else
   {
                ...
                (OPTION_REG)|=(0x20);
    au8_Timer_Started[0]=0;
   }
   break;
  case 1 :
   if(au8_Timer_Started[1] != 1)
   {
   ...
   {
    (T1CON)&=~(0x01);
    au8_Timer_Started[1]=0;
   }
   break;
  case 2 :
   if(au8_Timer_Started[2] != 1)
   {
   }
   else
   {
    (T1CON)&=~(0x04);
    au8_Timer_Started[2]=0;
   }
   break;
  default:
            break;
...
void Timer_GetTickTime(uint8 u8_Timer_CH_NO,uint8 *pu8_Time)
{
 *pu8_Time = au8_PrescalerValue[u8_Timer_CH_NO]/8;
}
# 527 "TIMER.c"
void Timer_SetValue(uint8 u8_Timer_CH_NO, uint16 u16_Timer_Count)
{
